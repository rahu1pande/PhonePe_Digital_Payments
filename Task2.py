...
#BAR Chart showing the distribution of different transactions types for each state for the most recent quarter in the dataset.
...
(
    ...
    .loc[
        lambda df: (
            (df["Year"] == df["Year"].max()) &
            (df["Quarter"] == df[df["Year"] == df["Year"].max()]["Quarter"].max())
        )
    ]
